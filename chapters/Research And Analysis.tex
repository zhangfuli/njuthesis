...
\begin{longtable}[h]{m{40pt}|m{40pt}|m{20pt}|m{150pt}|m{80pt}}
    \caption[基于Kubernetes Operator云化策略集]{基于Kubernetes Operator云化策略集} \label{policy_set} \\  
        ...
        \hline
        可迁移性 & 适应性
        &S1&容器化及Kubernetes的能力 & [P1-P3, P8, P10-P12] \\\cline{3-5}

        \hline
        \multirow{2}*{\parbox[c]{40pt}{生产效率 \\ 效率}} & \multirow{2}*{易用性}
        &S2&自动化配置领域知识 & [P1, P2, P12-15] \\\cline{3-5}
        & &S3&Operator与Helm结合 & [P1, P4, P13] \\\cline{3-5}

        \hline
        \multirow{2}*{可用性} & \multirow{2}*{可靠性}
        &S1&容器化及Kubernetes的恢复机制 & [P2, P15] \\\cline{3-5}
        & &S4&主备切换 & [P15] \\\cline{3-5}

        \hline
        \multirow{3}*{\parbox[c]{40pt}{可扩展性 \\ 可伸缩性 \\ 灵活性}} & \multirow{3}*{无}
        &S1&容器化及Kubernetes的多副本能力 & [P2, P3, P5, P7, P12, P15] \\\cline{3-5}
        & &S5&基于监控指标的伸缩处理 & [P2, P6, P13] \\\cline{3-5}
        & &S6&链外利用存储即服务 & [P15] \\\cline{3-5}

        \hline
        \multirow{4}*{安全性} & \multirow{4}*{安全性}
        &S7&基于角色的访问控制 & [P2] \\\cline{3-5}
        & &S8&多用户认证授权机制 & [P3, P13] \\\cline{3-5}
        & &S9&自定义访问控制机制 & [P9] \\\cline{3-5}
        & &S10&项目级别资源隔离 & [P9, P13] \\\cline{3-5}

        \hline
        \multirow{1}*{可监控性} & \multirow{1}*{无}
        &S11&基于Prometheus的监控体系 & [P1, P5, P12, P13] \\\cline{3-5}
        \hline
    \end{longtable} 
}
...
\textbf{适配性: }HF官方提供了Binaries和Docker镜像两种安装方式\footnotemark[2]\footnotetext[2]{\href{https://github.com/hyperledger/fabric/blob/main/scripts/bootstrap.sh}{fabric\_bootstrap.sh}}, 其天然的满足容器化构建。然而, 托管于Kubernetes需要为HF不同类型的网络节点提供多种的资源对象, 包含但不限于Deployment、Service等。一旦HF网络节点托管于Kubernetes, 即可原生的利用Kubernetes进行高可用的管理。
...
\textbf{适配性: }可以将HF网络节点通过CRD注册进入Kubernetes, 然而这存在一定的复杂性。编写HF领域CRD根据各网络节点的配置\footnotemark[3]\footnotetext[3]{\href{https://github.com/hyperledger/fabric-ca/blob/main/cmd/fabric-ca-server/config.go}{Ca Config}}\footnotemark[4]\footnotetext[4]{\href{https://github.com/hyperledger/fabric/blob/main/sampleconfig/orderer.yaml}{Orderer Config}}\footnotemark[5]\footnotetext[5]{\href{https://github.com/hyperledger/fabric/blob/main/sampleconfig/core.yaml}{Peer Config}}以及网络预期状态设计需要注入的属性。以Ca为例, 其存在22种一级配置项, 在编写Ca CRD时就需要对这些配置项进行甄别与筛选, 选取出适合插拔配置以及使用频率较高的配置并利用编程语言对其进行编写转换。
