...
% 论文的英文标题，不可换行
\englishtitle{}
\englishabstracttitlea{Research and implementation of blockchain cloudification framework}
\englishabstracttitleb{ for Hyperledger Fabric}
% 论文作者姓名的拼音
...
\begin{abstract}
  区块链具有去中心化、不可伪造、不可篡改的特性, 被誉为下一代的新型生产关系。区块链的发展为传统领域带来了巨大变革, 去中心化应用逐步从理论论证阶段转变为工程化实践阶段。BaaS作为基于云原生技术体系的一种构建、管理、托管和运维区块链网络及去中心化应用的云服务平台大大降低了去中心化应用落地的门槛。
  ...
  国内, 云原生生产环境中72\%的工程师使用Kubernetes作为云基础设施, 同时Kubernetes Operator是将领域知识集成到Kubernetes API编排过程中的最新方法。针对上述问题, 本文通过快速评审的方法, 调研学术界如何利用Kubernetes Operator有效赋能传统领域。最终整理出12条赋能策略集, 再结合区块链云化的原则以及区块链本身特性筛选并获得区块链云化实施方案。基于区块链云化实施方案本文设计实现了面向Hyperledger Fabric的区块链云化框架及其原型平台。

  本文通过典型案例分析对原型工具进行了功能性与非功能性测试。通过Kubernetes Operator五层成熟度模型进行了定性评估, 通过与官方BaaS平台Cello对比进行了定量评估。测试与评估结果表明, 本文提出的框架及原型平台能够兼容基本BaaS功能前提下充分利用云的特性提升Hyperledger Fabric网络的可扩展性、安全性以及可视运维能力, 基本满足五层成熟度的能力要求并且拥有更优的网络部署时间。
